  ...
  return (
    <div key={ann.id} style={{ background: idx === 0 ? (dark ? 'rgba(52,211,153,0.07)' : 'rgba(52,211,153,0.05)') : (dark ? 'rgba(255,255,255,0.02)' : 'rgba(0,0,0,0.02)'), border: `1px solid ${idx === 0 ? 'rgba(52,211,153,0.35)' : (dark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.08)')}`, borderRadius: '14px', padding: '16px 18px', position: 'relative' }}>
      ...
      {isMentioned && (
        ...
        >
          <div style={{ fontSize: '10px', color: '#34d399', padding: '3px 14px', border: '1px solid rgba(52,211,153,0.3)', borderRadius: '20px', cursor: 'default', background: 'rgba(52,211,153,0.06)', fontWeight: 600 }}>
            🎂 You are mentioned · {replies.length} wish{replies.length !== 1 ? 'es' : ''} — hover to see
          </div>
          {replyPopupAnnId === ann.id && (
            <div style={{ position: 'absolute', bottom: '28px', left: '50%', transform: 'translateX(-50%)', background: dark ? 'rgba(5,10,20,0.97)' : '#fff', border: '1px solid rgba(52,211,153,0.3)', borderRadius: '16px', padding: '14px 16px', minWidth: '260px', maxWidth: '340px', maxHeight: '260px', overflowY: 'auto', zIndex: 9999, boxShadow: '0 16px 48px rgba(0,0,0,0.6)', animation: 'fadeIn 0.2s ease' }}>
              <div style={{ fontSize: '10px', fontWeight: 800, color: '#34d399', letterSpacing: '1.5px', marginBottom: '10px' }}>💬 WISHES ({replies.length})</div>
              {replies.length === 0 ? (
                <div style={{ color: subtext, fontSize: '12px', textAlign: 'center', padding: '16px 0' }}>No wishes yet</div>
              ) : replies.map(r => (
                <div key={r.id} style={{ marginBottom: '8px', padding: '8px 10px', background: 'rgba(255,255,255,0.04)', borderRadius: '10px', border: '1px solid rgba(52,211,153,0.15)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                    <span style={{ fontSize: '10px', fontWeight: 700, color: '#34d399' }}>{r.replied_by_name}</span>
